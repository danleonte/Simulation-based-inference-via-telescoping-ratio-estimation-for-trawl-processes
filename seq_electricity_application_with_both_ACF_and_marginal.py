...
def run_analysis_for_dataset(dataset_name, data_type, base_path):
    ...
    #x = (x - jnp.mean(x, axis=1, keepdims=True)) / jnp.std(x, axis=1, keepdims=True) #demean and standardize
    ...
    num_replications_for_empirical_ACF_CI = 1000  # Adjust as needed
    ...
    # Create ACF plot with debiasing
    h = jnp.arange(0, nlags+1, 1)
    ...
    # Standard error bands for ACF (like R's acf function)
    n = len(x.squeeze())
    se_bands = 1.96 / np.sqrt(n)
    
    # Create single ACF plot
    fig, ax = plt.subplots(figsize=(10, 7))
    
    # Empirical ACF with CI
    ax.plot(h, empirical_acf, 'o-', alpha=0.9, label='Empirical ACF', 
            color='darkblue', markersize=4, zorder=3)
    ax.fill_between(h, empirical_lower, empirical_upper, alpha=0.2, color='darkblue', 
                    label='95% CI', zorder=1)
    
    # MAP ACF with theoretical CI from posterior
    ax.plot(h, map_acf, '-', linewidth=2, label='MAP ACF', color='red', zorder=3)
    ax.fill_between(h, acf_lower_95, acf_upper_95, alpha=0.2, color='red', 
                    label='95% CI (theoretical)', zorder=1)
    
    # Posterior mean and median
    ax.plot(h, posterior_mean, '--', alpha=0.7, label='Posterior mean', color='orange', linewidth=1.5)
    ax.plot(h, posterior_median, ':', alpha=0.7, label='Posterior median', color='green', linewidth=1.5)
    
    # Add standard error bands (like R)
    ax.axhline(y=se_bands, linestyle='--', color='gray', alpha=0.5, linewidth=0.8)
    ax.axhline(y=-se_bands, linestyle='--', color='gray', alpha=0.5, linewidth=0.8)
    ax.axhline(y=0, linestyle='-', color='black', alpha=0.3, linewidth=0.5)
    
    ax.set_xlabel('Lag', fontsize=12)
    ax.set_ylabel('ACF', fontsize=12)
    ax.set_title(f'Autocorrelation Function - {dataset_name} ({data_type})', fontsize=14)
    ax.legend(loc='upper right', fontsize=10)
    ax.grid(True, alpha=0.3)
    ax.set_ylim(-0.2, 1.0)
    
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'acf_comparison.png'), dpi=150)
    plt.close()
    ...
    data_array = np.array(x).squeeze()  # Use the original data_log values
    
    # Convert MAP marginal parameters to TensorFlow NIG parameterization
    map_marginal_params_nig = convert_3_to_4_param_nig(map_marginal_params_standardized)
    
    # Create range for PDF plotting based on actual data range
    x_min, x_max = data_array.min(), data_array.max()
    x_range = np.linspace(x_min , x_max , 10000)
    # Transform x_range to standardized scale for PDF evaluation
    x_range_standardized = (x_range - mean_x) / std_x
    
    # Compute NIG density on standardized scale
    try:
        map_nig_dist = tfp_dist.NormalInverseGaussian(*map_marginal_params_nig, validate_args=True)
        # Evaluate PDF on standardized scale
        map_pdf_standardized = map_nig_dist.prob(x_range_standardized)
        # Apply Jacobian to get PDF on original scale
    ...
    # Histogram
    ax.hist(data_array, bins=bins, density=True, alpha=0.3, color='gray', 
            edgecolor='black', linewidth=0.5)#, label=f'Histogram ({bins} bins)')
        
    # KDE
    sns.kdeplot(data=data_array, ax=ax, color='blue', linewidth=2, label='KDE', alpha=0.8)
        
    # MAP NIG density (now properly transformed)
    ax.plot(x_range, map_pdf, 'r-', linewidth=2, label='MAP NIG', alpha=0.8)
        
    ax.set_xlim(x_min - np.abs(x_min) / 100, x_max +  np.abs(x_max) / 100)
    ax.set_xlabel('Value', fontsize=12)
    ax.set_ylabel('Density', fontsize=12)
    ax.set_title('Marginal Density', fontsize=14)
    ax.legend(loc='best', fontsize=9)
    ax.grid(True, alpha=0.3)
    
    #plt.suptitle(f'Marginal Distribution - {dataset_name} ({data_type})', fontsize=14, y=1.02)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'marginal_density_comparison.png'), dpi=300)
    plt.close()
