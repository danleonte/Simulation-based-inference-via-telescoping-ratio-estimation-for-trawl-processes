...
def run_analysis_for_dataset(dataset_name, data_type, base_path):
    ...
    for tre_type in tre_types_list:
        ...
        calibration_file = os.path.join(trained_classifier_path, f'beta_calibration_{2000}.pkl')
    ...
    print(f"Estimating CI for {dataset_name} ({data_type})...")
    ...
    map_acf_at_20 = corr_sup_ig_envelope(jnp.arange(0, 50, 1), results_MAP[:2])
    effective_decorr_lag = np.where(np.array(map_acf_at_20) < 0.05)[0]
    if len(effective_decorr_lag) == 0:
        raise ValueError(f"MAP ACF does not decay below 0.05 within 20 lags for {dataset_name} ({data_type})")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'marginal_acf.png'), dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.suptitle(f'Parameter Posteriors', fontsize=14)
    plt.tight_layout()
        ...
        for data_type in ['MSTL', 'R_deseasonalized']:
            if dataset_name in all_results and data_type in all_results[dataset_name]:
                ...
                if data_type == 'MSTL':
                    ...
                    save_path = os.path.join(data_base, 'R_deseasonalized_data', dataset_name)
