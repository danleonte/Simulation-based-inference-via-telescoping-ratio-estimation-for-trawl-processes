...
save_file_path = os.path.join(folder_path, str(seq_len) + calibrate_suffix)
...
if False:
    ...
    # 'o-', label='Empirical calibration', markersize = 5)
    plt.plot(theoretical_coverages, empirical_coverages,
             label='Empirical calibration')
    # '--', label='Perfect calibration', markersize = 5)
    plt.plot(theoretical_coverages, theoretical_coverages,
    ...
    output_path = os.path.join(
        parent_dir, f'coverage_check_{num_points}_{seq_len}_{calibrate_suffix}_{double_cal}.pdf')
