...
class TransformerBlock(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, deterministic: bool = True) -> jnp.ndarray:
        ...
        if self.freq_attention:
            attn_output = self.frequency_attention(x, deterministic)
        else:
        ...
        # MLP block
        mlp_output = self.mlp_block(x, deterministic)
    ...
    @nn.compact
    def frequency_attention(self, x: jnp.ndarray, deterministic: bool) -> jnp.ndarray:
        """Attention in frequency domain."""
        # Get input shape
        batch_size, seq_len, channels = x.shape

        # Convert to frequency domain - handle batched input properly
        x_freq = jnp.fft.rfft(x, axis=1)  # No need for vmap here
        ...
        output = jnp.fft.irfft(attn_output, n=seq_len,
                               axis=1)  # No need for vmap
    ...
    @nn.compact
    def mlp_block(self, x: jnp.ndarray, deterministic: bool) -> jnp.ndarray:
