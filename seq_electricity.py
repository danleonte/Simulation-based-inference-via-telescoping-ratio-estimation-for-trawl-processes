...
from src.utils.estimate_bias import estimate_bias_from_MAP
...
def run_analysis_for_dataset(dataset_name, data_type, base_path):
    ...
    empirical_acf = sm.tsa.stattools.acf(np.array(x.squeeze()), nlags=40)[1:]
    ...
    posterior_std = np.std(np.array(acf_curves), axis=0)
    
    # MAP ACF
    map_params = results_MAP[:2]
    map_acf = corr_sup_ig_envelope(h, map_params)
    
    # Create comprehensive ACF plot
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    
    # Plot 1: Mean with uncertainty bands
    axes[0].plot(h, empirical_acf, 'o-', alpha=0.7, label='Empirical ACF')
    axes[0].plot(h, posterior_mean, '--', alpha=0.7, label='Posterior mean ACF')
    axes[0].fill_between(h, posterior_mean - 2*posterior_std, posterior_mean + 2*posterior_std, 
                         alpha=0.3, label='±2 std')
    axes[0].set_title(f'ACF Comparison (Mean) - {dataset_name} ({data_type})')
    ...
    # Plot 2: Median
    axes[1].plot(h, empirical_acf, 'o-', alpha=0.7, label='Empirical ACF')
    axes[1].plot(h, posterior_median, '--', alpha=0.7, label='Posterior median ACF')
    axes[1].set_title(f'ACF Comparison (Median) - {dataset_name} ({data_type})')
    ...
    # Plot 3: MAP
    axes[2].plot(h, empirical_acf, 'o-', alpha=0.7, label='Empirical ACF')
    axes[2].plot(h, map_acf, '--', alpha=0.7, label='MAP ACF')
    axes[2].set_title(f'ACF Comparison (MAP) - {dataset_name} ({data_type})')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'acf_comparison_all.png'), dpi=150)
    plt.close()
