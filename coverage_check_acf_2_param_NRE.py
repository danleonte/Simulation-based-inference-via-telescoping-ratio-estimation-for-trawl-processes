...
if __name__ == '__main__':
    ...
    # this is an artefact from an older version.
    # in fact, we should pre-compute the val_x_cache to avoid recomputing it
    ...
    assert os.path.exists(val_x_cache_path)
   ...
   #     jnp.array(val_x[[0]]), jnp.array(val_thetas[[0]]))
    ...
    if calibration_type in ['isotonic', 'beta']:
        file_path = f'{tre_type}_cal_ranks_{calibration_type}_seq_len_{seq_len}_N_{N}.npy'
    else:
